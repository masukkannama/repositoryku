...
async function handler(req: Request) {
  ...
  if (path[0] === "json" && path[1] === "list") {
    const type = path[2] || "day"; // day/week/month/search/category/tag/maker/cast
...
function makeListURL(type: string, param: string) {
  if (type === "day" || type === "week" || type === "month") {
    ...
    return new URL(`/en/tag/${param}`, "https://supjav.com");
  } else if (type === "maker") {
    return new URL(`/en/category/maker/${param}`, "https://supjav.com");
  } else if (type === "cast") {
    return new URL(`/en/category/cast/${param}`, "https://supjav.com");
  }
