...
// Pertimbangkan juga untuk menggunakan referer milik supjav sendiri jika yang di atas tidak bekerja
...
async function handler(req: Request) {
  ...
  // Bagian untuk proxy penuh (mengatasi masalah referer dan hotlinking)
  if (uri.pathname.match(/^\/.*?\/stream\//)) {
    ...
    const res = await fetch(u, {
      headers: {
        "Referer": u.origin, // Referer untuk CDN video biasanya adalah asal CDN itu sendiri
        "User-Agent": UA,    // Juga sertakan UA untuk permintaan ke CDN
      },
    });
  ...
  // ... (Bagian lainnya seperti popular, search, category tetap sama) ...
...
export async function getM3U8ById(id: string) {
  const req1 = await fetch(`https://supjav.com/${id}.html`, {
    headers: {
      "Referer": REFERER, // Gunakan constant yang telah didefinisikan
      "User-Agent": UA,
    },
  });
...
// FUNGSI YANG DIPERBAIKI: Tambahkan headers ke semua permintaan ke supjav.com
async function fetchBody(url: string | URL) {
  const req = await fetch(url, {
    headers: {
      "User-Agent": UA,
      "Referer": REFERER, // INI PENYEBAB UTAMA KEGAGALAN SEBELUMNYA
    },
  });
