...
async function handler(req) {
  try {
    ...
    // Endpoint daftar + m3u8 per halaman
    if (uri.pathname.match(/^\/json\/(day|week|month|search|category|maker|cast|tag)/)) {
      ...
      if (page < 1) return new Response("Page must be >= 1", { status: 400 });
      ...
      // ambil daftar 1 halaman saja
      const list = await getListOnePage(base, page);
...
async function getListOnePage(base, page = 1) {
  ...
  const mediaList = extractMediaList(html) || [];

  // Ambil m3u8 secara paralel (tapi tetap terbatas hanya 1 halaman)
