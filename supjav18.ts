...
async function handler(req) {
  try {
    ...
    // === endpoint list (day, week, month, search, category, dll) ===
    ...
    if (uri.pathname.startsWith("/json/all/")) {
      ...
      if (ids.length > 20) {
        return new Response("Max 20 IDs allowed per request", { status: 400 });
      }
      ...
      const results = await Promise.all(
        ids.map(async (id) => {
          ...
          // ambil thumbnail (coba beberapa pola)
          let thumb =
            html.match(/data-original="([^"]+)"/)?.[1] ||
            html.match(/data-src="([^"]+)"/)?.[1] ||
            html.match(/src="([^"]+\.jpg)"/)?.[1] ||
            "";
          ...
          if (linkList) {
            ...
            if (firstKey) {
              ...
              m3u8Url = data2.match(/urlPlay.*?(https.*?\.m3u8)/)?.[1] || "";
            }
          }
