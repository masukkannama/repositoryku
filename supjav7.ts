...
async function handler(req: Request) {
  ...
  // Bagian untuk tautan yang mengarah ke Worker untuk mem-proxy video
  if (uri.pathname.match(/^\/\d+(\.m3u|\.html|\.m3u8)?$/)) {
    ...
    return new Response(null, {
      status: 307,
      headers: { location: `/${urlobj.hostname}/stream${urlobj.pathname}` },
    });
  }

  // Bagian untuk membuat playlist M3U (bagian ini yang Anda akses pertama)
